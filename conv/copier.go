...
const copyTagName = "copier"

// Copy copy things
func Copy(fromValue interface{}, toValue interface{}, mapping ...map[string]string) (err error) {
	var (
		isSlice bool
		amount  = 1
	...
	var tagName = copyTagName

	if !to.CanAddr() {
		return errors.New("copy to value is unaddressable")
	}
	var mapField = make(map[string]string)
	if mapping != nil {
		mapField = mapping[0]
	}
	...
	for i := 0; i < amount; i++ {
		...
		if dest.IsValid() {
			tagBitFlags = getBitFlags(toType, tagName)
		}
		...
		if source.IsValid() {
			...
			for _, field := range fromTypeFields {
				...
				if fromField := source.FieldByName(name); fromField.IsValid() {
					...
					var ok bool
					if toFieldName, ok = mapField[name]; !ok {
						toFieldName = name
					}
					if toField := dest.FieldByName(toFieldName); toField.IsValid() {
						if toField.CanSet() {
...
func getBitFlags(toType reflect.Type, tagName string) map[string]uint8 {
